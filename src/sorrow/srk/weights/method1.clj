...
(defn- weight-parameters
  "For a desired encoded word length n and alphabet size p (prime), calculate the
   ...
   encoding and decoding.  Returns a map of the parameters :a, :b."
  [p n]
  {:a (- n 2)
   :b (->> (range p)
...
(defn weight-scheme
  "For a desired encoded word length n and alphabet size p (prime), calculate the
   weight parameters and sequences that will be used in encoding and decoding.
   Returns a map of the weight parameters :a, :b and the weight sequences :w, :w'."
  [p n]
  (let [{:keys [a b]} (weight-parameters p n)
        r (range 1 (inc n))
        w (map (partial + a) r)
        w' (map #(mod (* (+ a %) (+ b %)) p) r)]
    {:a a :b b :w w :w' w'}))
